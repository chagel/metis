...
```
   ...
   Agent service layer  (app/services/agent/)
      ...
      └─ Runtime        where pi runs — Local · Docker (gVisor) · E2b · Daytona · Microsandbox
      │
   ...
   Persistence
      ...
      └─ Workspace fs     Docker/Microsandbox bind mount · E2b/Daytona pause/resume · Local host
```
...
- [`coding-runtime.md`](coding-runtime.md) — the `local` / `docker` / `e2b` / `daytona` / `microsandbox` runtimes (prod: `docker` under gVisor).
