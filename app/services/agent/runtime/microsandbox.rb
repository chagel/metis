...
module Agent
  module Runtime
    ...
    # `rake runtime:image` pushes the built tag to the configured ref. Build
    # it for the *worker's* arch: libkrun boots the guest on the host CPU, so
    # unlike the hosted runtimes (which build on the provider's fleet) a
    # mismatch here fails the same way Runtime::Docker's does.
    #
